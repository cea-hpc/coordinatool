...
archive_on_hosts_ch() {
	...
	do_client 1 "[ \"\$(find ${ARCHIVEDIR@Q}/1 | wc -l)\" = \"\$(($nb_test1 + 1))\" ]" \
		|| error "missing archives on 1 or unexpected archives on 1"
	do_client 2 "[ \"\$(find ${ARCHIVEDIR@Q}/2 | wc -l)\" = \"\$(($nb_test2 + 1))\" ]" \
	...
	local nb_test0=$(find ${ARCHIVEDIR}/0 -name "grouping=test0*" | wc -l)

	do_client 0 "[ \"\$(find ${ARCHIVEDIR@Q}/0 | wc -l)\" = \"\$(($nb_test0 + 1))\" ]" \
