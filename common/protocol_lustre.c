...
{
	return json_pack("{sI,si,si}", "f_seq", fid->f_seq, "f_oid", fid->f_oid,
			 "f_ver", fid->f_ver);
}
...
int json_fid_get(json_t *json, struct lu_fid *fid)
{
	...
	rc = json_unpack(json, "{sI,si,si!}", "f_seq", &unpack.f_seq, "f_oid",
			 &unpack.f_oid, "f_ver", &unpack.f_ver);
	if (rc)
		return rc;
...
json_t *json_hsm_action_item(struct hsm_action_item *hai, uint32_t archive_id,
			     uint64_t flags)
{
	return json_pack("{si,so,so,sI,sI,sI,sI,sI,sI,ss#}", "hai_action",
			 hai->hai_action, "hai_fid", json_fid(&hai->hai_fid),
			 "hai_dfid", json_fid(&hai->hai_dfid),
			 "hai_extent_offset", hai->hai_extent.offset,
			 "hai_extent_length", hai->hai_extent.length,
			 "hai_cookie", hai->hai_cookie, "hai_gid", hai->hai_gid,
			 "hal_archive_id", archive_id, "hal_flags", flags,
			     ...
			     size_t hai_len, const char **pdata)
{
	...
	if (json_unpack(json, "{si,s{sI,si,si!},s{sI,si,si!},sI,sI,sI,sI,ss%}",
			"hai_action", &unpack.hai_action, "hai_fid", "f_seq",
			&unpack.hai_fid.f_seq, "f_oid", &unpack.hai_fid.f_oid,
			"f_ver", &unpack.hai_fid.f_ver, "hai_dfid", "f_seq",
			&unpack.hai_dfid.f_seq, "f_oid", &unpack.hai_dfid.f_oid,
			"f_ver", &unpack.hai_dfid.f_ver, "hai_extent_offset",
			&unpack.hai_extent.offset, "hai_extent_length",
			&unpack.hai_extent.length, "hai_cookie",
			&unpack.hai_cookie, "hai_gid", &unpack.hai_gid,
			    ...
			    struct lu_fid *hai_dfid)
{
	...
	struct lu_fid_unpacked unpack;
	if (json_unpack(json, "{sI,s{sI,si,si!}}", "hai_cookie", hai_cookie,
			"hai_dfid", "f_seq", &unpack.f_seq, "f_oid",
			&unpack.f_oid, "f_ver", &unpack.f_ver) != 0)
		return -EINVAL;
			     ...
			     size_t hal_len, hal_get_cb cb, void *cb_arg)
{
	...
	{
		if ((rc = json_hsm_action_item_get(item, hai, hal_len, NULL)) <
		    0)
			return rc;
