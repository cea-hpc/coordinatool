...
int report_new_action(struct hsm_action_node *han)
{
	...
	int data_len = han_data_len(han), needle_len = strlen(hint_needle);
		  ...
		  struct client *client)
{
	...
	case REPORT_NEW_REQUEST:
		n = snprintf(buf, sizeof(buf), "fid " DFID " received\n",
			     PFID(&han->info.dfid));
		break;
	case REPORT_ASSIGN_REQUEST:
		assert(client);
		n = snprintf(buf, sizeof(buf), "fid " DFID " assigned %s\n",
			     PFID(&han->info.dfid), client->id);
		break;
	case REPORT_SEND_REQUEST:
		assert(client);
		n = snprintf(buf, sizeof(buf), "fid " DFID " sending %s\n",
			     PFID(&han->info.dfid), client->id);
		break;
	case REPORT_MOVER_PROGRESS:
		assert(client);
		n = snprintf(buf, sizeof(buf), "mover %s progressed\n",
			     client->id);
		break;
	}
	if (n < 0) {
